...
import static com.pumpkin.disease.constant.RedisPrefixConstant.EmailCaptchaPrefix.CHANGE_PASSWORD_CAPTCHA;
import static com.pumpkin.disease.constant.StrategyServiceConstant.EmailCaptcha.CHANGE_PASSWORD_EMAIL_CAPTCHA_SERVICE_NAME;

/**
 * @author: Pumpkin
 * @description: 修改密码邮箱验证码策略实现类
 * @since : 2023/4/23 14:22
 */
@RequiredArgsConstructor
@Service(value = CHANGE_PASSWORD_EMAIL_CAPTCHA_SERVICE_NAME)
public class ChangePasswordEmailCaptchaStrategyContextImpl implements EmailStrategy {
    ...
    public void sendEmailToUser(EmailRequest emailRequest) {
        ...
        // 存储验证码
        redisCache.setCacheObject(CHANGE_PASSWORD_CAPTCHA + emailRequest.getReceiverEmail(), emailCaptcha, 15, TimeUnit.MINUTES);
        // 发送邮箱
        emailService.sendEmailToUserByEmailTemplate(emailRequest.getReceiverEmail(),
                EmailEnum.CHANGE_PASSWORD_EMAIL_CAPTCHA.getEmailMessage(),
                EmailEnum.CHANGE_PASSWORD_EMAIL_CAPTCHA.getEmailMessage(),
    ...
    public void checkEmailCaptcha(EmailCaptchaRequest emailCaptchaRequest) {
        String registerCaptcha = redisCache.getCacheObject(CHANGE_PASSWORD_CAPTCHA + emailCaptchaRequest.getReceiverEmail());
        AssertUtil.assertIsNotPass(!emailCaptchaRequest.getEmailCaptcha().equals(registerCaptcha), EMAIL_CAPTCHA_ERROR);
        redisCache.deleteObject(CHANGE_PASSWORD_CAPTCHA + emailCaptchaRequest.getReceiverEmail());
    }
}
